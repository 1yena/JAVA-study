...
public class GamePanel extends JPanel {
	...
	private final static int CELLSIZE = 30;		// 글자의 크기
		...
		addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				...
				if(row >= world.getRows() || col >= world.getColumns()) {
					return;		// 벗어난 값이기 때문에 그냥 리턴해줌.(큰값)
				}
				Boolean status = world.getCell(row, col);	
				world.setCell(row, col, !status);	// 현재 녹색인지 체크해서 반전시킴.
...
//		System.out.println(cols);
		...
		//world.setCell(0, 0, true);	// grid 이중배열에 좌표(줄, 열) 값을 true로 set하다. ***true
	...
	private void fillCell(Graphics2D g2, int row, int col, boolean status) {
		// 사각형에 색을 넣는 메소드( 그래픽객체, 가로즐, 세로줄, 상태(true=녹색, false=배경색) )
		Color color = status ? Color.PINK : backgroundColor;	// 삼항연산자 status가 true면 녹색
		g2.setColor(color);
		
		int x = leftRightMargin + (col * CELLSIZE);
		int y = topBottomMargin + (row * CELLSIZE);
		
		// x,y좌표 가로세로 길이 입력해서 사각형에 색을 칠한다.
	...
	public void clear() {
		// 백스페이스 눌렀을 때 모든 셀을 false(검은색)로 초기화
		world.clear();
		// 리프레쉬
		repaint(); 	// 검은색으로 초기화.
