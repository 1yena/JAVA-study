...
implements ActionListener, Runnable {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(obj==bt1/*connect*/) {
			...
			try {
				FileWriter fw = new FileWriter("jsp_ch14/"+fileName+".txt");
				fw.write(content);
				fw.close();
				area.setText("");
			...
			if(filterMgr(str)) {
				...
				return; //메소드 중간에 빠져나가는 결과. 즉 밑에 소스는 의미 없어짐.
			}
			if(!flag/*아이디 입력*/) {
				sendMessage(ChatProtocol2.ID + ":" + str); //서버 전송
				...
				if(i==-1 || i==0) {//전체 채팅
					...
					String id = list.getSelectedItem(); //선택한 id값을 리턴
	...
	public boolean filterMgr(String msg){
		...
		//msg : 하하 호호 히히
		StringTokenizer st = new StringTokenizer(msg); //생략하면 구분자는 공백임.
