...
import jsp_ch14.ChatServer1.ClientThread1;
...
public class ChatServer2 {
	...
	public ChatServer2() {
		...
		try {
			while (true) {
				Socket sock = server.accept(); //클라이언트 접속대기 상태입니다.
				ClientThread2 ct = new ClientThread2(sock);
				ct.start(); //Thread 스케쥴러에게 등록 -> 스케쥴러가 시작을 하는 단계.
				vc.addElement(ct); //벡터에 추가함
			}
		} catch (Exception e) {
...
//				e.printStackTrace();
		...
		if(cmd.equals(ChatProtocol2.ID)) {
			...
			if(data!=null && data.length()>0) {
				id = data;
				//새로운 접속자가 추가되었기 떄문에 리스트를 재전송해줘야 함
